...
let secret = "HCFQQARAAHRGMYDK"; // This is the secret key that is used to generate the OTP
let secret2 = "MIMUKNPMLUFEFSCY";
...
export const generatesecondOtpAndEncryptBody = (body) => {
  const time = Math.floor(new Date().getTime() / 1000);
  const interval = Math.floor(time / 30).toString(16).padStart(16, "0");
  const otp = cryptoJSTotp(interval, secret);
  console.log("Pre-Request OTP", otp);
  console.log("Pre-Request SECRETKEY", secret2);
