...
test.describe.parallel('API Testing WhoAmI', () => {
  ...
  test('WhoAmI 10 times', async ({ request }) => {
   ...
   test('WhoAmI with large payload', async ({ request }) => {
    const bearerToken = getEnvVariable('bearer_token');
    const newSecret = getEnvVariable('secret_key');

    console.log("Using new secret key TOTP for WhoAmI request with large payload.");

    // Generate a large payload
    const largePayload = { data: 'A'.repeat(10 * 1024 * 1024) }; // 10 MB payload

    const { encryptedBody, otp: newOtp } = generateOtpAndEncryptBody(largePayload, newSecret);

    setEnvVariable('generated_totp_code', newOtp);

    const response = await request.post(`${baseURL}/auth/whoami`, {
      data: { data: encryptedBody },
      headers: {
        'Authorization': `Bearer ${bearerToken}`,
        'Content-Type': 'application/json'
