...
test.describe.parallel('API Testing - Person Profile', () => {
    ...
    test('Get Profile - without Bearer Token', async ({ request }) => {
      ...
      expect(response.status()).toBe(403); // Assuming the API returns 401 for unauthorized access
    });
    ...
    test('Get Profile - Invalid Bearer Token', async ({ request }) => {
      ...
      expect(response.status()).toBe(502); // Assuming the API returns 401 for invalid token
    });
    ...
    test('Get Profile - Expired Token', async ({ request }) => {
      ...
      expect(response.status()).toBe(502); // Assuming the API returns 401 for expired token
    });
  });