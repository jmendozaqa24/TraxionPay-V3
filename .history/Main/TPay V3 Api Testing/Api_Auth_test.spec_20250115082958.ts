...
test.describe.parallel('API Testing - Refresh Tokens', () => {
 ...
 test('Refresh Token', async ({ request }) => {
  ...
  const loginResponseData = decryptResponse(loginResponseBody, otp);

  // Store environment variables
  if (loginResponseData.code == '220022') {
    setEnvVariable('bearer_token', loginResponseData.data.accessToken);
    setEnvVariable('refresh_token', loginResponseData.data.refreshToken);
    setEnvVariable('secret_key', loginResponseData.data.secretKey);
    setEnvVariable('generated_totp_code', otp); // Save the OTP used for login
  }
  ...
  const refreshResponse = await request.post(`${baseURL}/auth/refresh-token`, {
    data: { refreshToken },
    headers: {
      'Content-Type': 'application/json'
    }
  ...
  // Decrypt the response
  const decryptedRefreshResponse = decryptResponse(refreshResponseBody, otp);
  console.log(decryptedRefreshResponse);

  // Store the new tokens
  if (decryptedRefreshResponse.code == '220022') {
    setEnvVariable('bearer_token', decryptedRefreshResponse.data.accessToken);
    setEnvVariable('refresh_token', decryptedRefreshResponse.data.refreshToken);
  }
});
});