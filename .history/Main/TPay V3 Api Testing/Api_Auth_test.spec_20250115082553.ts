...
test.describe.parallel('API Testing - Refresh Tokens', () => {
 const baseURL = 'https://api-sit.traxionpay.com';
  ...
  test('Refresh Token', async ({ request }) => {
    ...
    const response = await request.get(`${baseURL}/auth/refresh-token`, {
      data: { refreshToken },
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    if (responseBody.code == '220022') {
      setEnvVariable('bearer_token', responseBody.data.accessToken);
      setEnvVariable('refresh_token', responseBody.data.refreshToken);
    }
  });
});