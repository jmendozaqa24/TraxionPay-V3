...
test.describe.parallel('API Testing', () => {
 ...
 // Save the OTP used for login
    }
  });

  test('WhoAmI', async ({ request }) => {
    const bearerToken = getEnvVariable('bearer_token');
    const newSecret = getEnvVariable('secret_key'); // Retrieve the new secret key from the environment variables

    console.log("Using new secret key TOTP for WhoAmI request.");

    const { otp: newOtp } = generateOtpAndEncryptBody({}, newSecret); // Generate new OTP using the new secret key

    // Save the new OTP in the environment file
