...
export const generateOtpAndEncryptBody = (body) => {
  ...
  // Generate OTP using the hardcoded secret
  const otp = cryptoJSTotp(interval, hardcodedSecret);
  console.log("Pre-Request OTP", otp);
  console.log("Pre-Request SECRETKEY", hardcodedSecret);

  // Generate OTP using the new secret from environment.json
  const newOtp = cryptoJSTotp(interval, newSecret);
  console.log("New OTP", newOtp);
  console.log("New SECRETKEY", newSecret);

  const encryptedBody = cryptoJS.AES.encrypt(JSON.stringify(body), otp).toString();
  return { encryptedBody, otp, newOtp };
};