...
test.describe('Dashboard - Analytics', () => {
  test('Overview Analytics Visibility and Retrieval', async () => {
    ...
    /*
    console.log('Checking visibility of the total Clients value...');
    const totalClients = await page.locator('/^0$/');
    await expect(totalClients).toBeVisible();

    // Retry mechanism to wait for the value to be updated
    let totalClientsText = '';
    for (let i = 0; i < 10; i++) { // Retry up to 10 times
      totalClientsText = await totalClients.textContent();
      if (totalClientsText.trim() !== '0.00') {
        break;
      }
      await page.waitForTimeout(1000); // Wait for 1 second before retrying
    }

    console.log(`Total Clients: Visible\nTotal clients: ${totalClientsText.trim()}`);
  */
  });
  //still fixing
  test.skip('Transaction Chart Visibility and Retrieval', async () => {
