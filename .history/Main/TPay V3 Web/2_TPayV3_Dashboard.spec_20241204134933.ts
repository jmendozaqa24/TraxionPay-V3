...
test.describe('Dashboard - Analytics', () => {
  test('Transaction Analytics Visibility', async () => {
    await page.getByText('PayIns 1,500.00').click();
    await page.locator('#SvgjsSvg2135 foreignobject').click();
    const entriesDropdown = await page.getByLabel('entries per page');
    const entriesOptions = ['Today', 'Last 7 days', 'Last 30 days', 'Last 3 months'];
    await page.getByText('PayOuts 1,005,400.00').click();
    await page.locator('#SvgjsSvg2033 foreignobject').click();
    const entriesDropdown = await page.getByLabel('entries per page');
    const entriesOptions = ['Today', 'Last 7 days', 'Last 30 days', 'Last 3 months'];
    await page.getByText('Clients 0').click();
    await page.locator('#SvgjsSvg2278 foreignobject').click();
    const entriesDropdown = await page.getByLabel('entries per page');
    const entriesOptions = ['Today', 'Last 7 days', 'Last 30 days', 'Last 3 months'];
    await page.getByText('Users 0').click();
    await page.locator('#SvgjsSvg2239 foreignobject').click();
    const entriesDropdown = await page.getByLabel('entries per page');
    const entriesOptions = ['Today', 'Last 7 days', 'Last 30 days', 'Last 3 months'];
