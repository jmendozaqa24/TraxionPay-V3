...
test.describe('TPay V3 - User Profile', () => {
  test('User Profile', async () => {
    ...
    const mobileIsActive = await page.locator('div:has-text("Type: Mobile Number") >> text=IsActive:').textContent();
    const mobileIsPrimary = await page.locator('div:has-text("Type: Mobile Number") >> text=IsPrimary:').textContent();

    const emailType = await getTextContent('Type: Email Address');
    const emailValue = await getTextContent('Value: ');
    const emailIsActive = await page.locator('div:has-text("Type: Email Address") >> text=IsActive:').textContent();
    const emailIsPrimary = await page.locator('div:has-text("Type: Email Address") >> text=IsPrimary:').textContent();
    ...
    console.log(`Mobile IsActive: ${mobileIsActive?.trim()}`);
    console.log(`Mobile IsPrimary: ${mobileIsPrimary?.trim()}`);
    console.log(`Email Type: ${emailType}`);
    console.log(`Email Value: ${emailValue}`);
    console.log(`Email IsActive: ${emailIsActive?.trim()}`);
    console.log(`Email IsPrimary: ${emailIsPrimary?.trim()}`);
