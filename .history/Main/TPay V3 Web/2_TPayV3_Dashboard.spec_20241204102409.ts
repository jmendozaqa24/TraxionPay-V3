...
test('Page Number Functionality', async () => {
  ...
  let showingText = generateShowingText(1, 10, totalEntries);
  ...
  await new Promise(resolve => setTimeout(resolve, 1000)); // Wait for the table to update
  showingText = generateShowingText(11, 20, totalEntries);
  ...
  await page.getByLabel('Next').click();
  await new Promise(resolve => setTimeout(resolve, 1000)); // Wait for the table to update
  showingText = generateShowingText(21, 30, totalEntries);
  showingElement = await page.getByText(showingText);
  expect(showingElement).toBeVisible();
  ...
  await new Promise(resolve => setTimeout(resolve, 1000)); // Wait for the table to update
  showingText = generateShowingText(1, 10, totalEntries);
  showingElement = await page.getByText(showingText);
  expect(showingElement).toBeVisible();
  ...
  await new Promise(resolve => setTimeout(resolve, 1000)); // Wait for the table to update
  const lastPageStart = Math.floor((totalEntries - 1) / 10) * 10 + 1;
