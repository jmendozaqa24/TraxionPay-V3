...
test.describe('Dashboard - Analytics', () => {
  ...
  test('Transaction Chart Visibility and Retrieval', async ({ page }) => {
    ...
    const chartSvg = await page.locator('#SvgjsSvg3704');
    await chartSvg.scrollIntoViewIfNeeded();
    await expect(chartSvg).toBeVisible();

    // Check if the bar graphs are visible and retrieve their attributes
    const barGraphs = await page.locator('#SvgjsSvg3704 foreignobject');
    const barGraphCount = await barGraphs.count();

    // Retrieve the heights of all bar graphs to find the maximum height
    let maxBarGraphHeight = 0;
    const barGraphHeights = [];
    for (let i = 0; i < barGraphCount; i++) {
      ...
      barGraphHeights.push(heightValue);
      if (heightValue > maxBarGraphHeight) {
        maxBarGraphHeight = heightValue;
      }
    }

    // Log the height and percentage of each bar graph
    for (let i = 0; i < barGraphCount; i++) {
      const barGraph = barGraphs.nth(i);
      await barGraph.scrollIntoViewIfNeeded();
      await expect(barGraph).toBeVisible();

      const barGraphHeight = barGraphHeights[i];
      const barGraphWidth = await barGraph.getAttribute('width');
      const percentage = ((barGraphHeight / maxBarGraphHeight) * 100).toFixed(2);
      console.log(`Bar Graph ${i + 1}: Height = ${barGraphHeight}, Width = ${barGraphWidth}, Percentage = ${percentage}%`);
    }
  });
});
