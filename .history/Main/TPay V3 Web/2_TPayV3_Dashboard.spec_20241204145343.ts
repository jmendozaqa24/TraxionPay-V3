...
test.describe('Dashboard - Analytics', () => {
  test('Overview Analytics Visibility and Retrieval', async () => {
    // Check if the "PayIns:" text is visible
    const payInsText = await page.getByText('PAYINS');
    await expect(payInsText).toBeVisible();

    // Check if the total PayIns value element is visible using a regular expression
    console.log('Checking visibility of the total PayIns value...');
    const totalPayIns = await page.getByRole('link', { name: /PayIns Today Today Last 7 days Last 30 days Last 3 months 1,500.00/ }); // Adjust the regex to match the format of the PayIns value
    await expect(totalPayIns).toBeVisible();
    const totalPayInsText = await totalPayIns.textContent();
    console.log(`Total PayIns: Visible\nPAYINS ${totalPayInsText.trim()}`);

    // Check if the dropdown links for different time periods are visible
    const todayLink = await page.getByRole('link', { name: 'Today' }).first();
    await expect(todayLink).toBeVisible();

    const last7DaysLink = await page.getByRole('link', { name: 'Last 7 days' }).nth(1);
    await expect(last7DaysLink).toBeVisible();

    const last30DaysLink = await page.getByRole('link', { name: 'Last 30 days' });
    await expect(last30DaysLink).toBeVisible();

    const last3MonthsLink = await page.getByRole('link', { name: 'Last 3 months' });
    await expect(last3MonthsLink).toBeVisible();
  });
});
