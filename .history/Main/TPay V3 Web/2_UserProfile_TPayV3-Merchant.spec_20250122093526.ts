...
test.describe('TPay V3 - User Profile', () => {
  ...
  test.describe('TPay V3 - Settings Tab', () => {
    ...
    test('Merchant Details Tab', async () => {
      ...
      const businessName = await page.getByText('Business Name:').nextSibling().textContent();
      console.log(`Business Name: ${businessName.trim()}`);
  
      const businessNameAlias = await page.getByText('Business Name Alias:').nextSibling().textContent();
      console.log(`Business Name Alias: ${businessNameAlias.trim()}`);
  
      const websiteURL = await page.getByText('Website URL:').nextSibling().textContent();
      console.log(`Website URL: ${websiteURL.trim()}`);
  
      const dateOfIncorporation = await page.getByText('Date Of Incorporation:').nextSibling().textContent();
      console.log(`Date Of Incorporation: ${dateOfIncorporation.trim()}`);
  
      const merchantCode = await page.getByText('Merchant Code:').nextSibling().textContent();
      console.log(`Merchant Code: ${merchantCode.trim()}`);
  
      const gcSubMID = await page.getByText('GC-subMID:').nextSibling().textContent();
      console.log(`GC-subMID: ${gcSubMID.trim()}`);
  
      const businessType = await page.getByText('Business Type:').nextSibling().textContent();
      console.log(`Business Type: ${businessType.trim()}`);
  
      const merchantType = await page.getByText('Merchant Type:').nextSibling().textContent();
      console.log(`Merchant Type: ${merchantType.trim()}`);
  
      const notesRemarks = await page.getByText('Notes/Remarks:').nextSibling().textContent();
