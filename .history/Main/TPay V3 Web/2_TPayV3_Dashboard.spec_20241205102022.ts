...
test.describe('Dashboard - Analytics', () => {
  ...
  test('Transaction Chart Visibility and Retrieval', async ({ page }) => {
    ...
    const chartSvg = await page.locator('#SvgjsSvg1245 foreignobject');
    await expect(chartSvg).toBeVisible();

    // Retrieve Y-axis labels and their values
    const yAxisLabels = await page.locator('#SvgjsSvg1245 text').filter({ hasText: /^[0-9]+$/ });
    const yAxisValues = [];
    for (let i = 0; i < await yAxisLabels.count(); i++) {
      const label = yAxisLabels.nth(i);
      const value = parseInt(await label.textContent(), 10);
      yAxisValues.push(value);
    }

    // Retrieve X-axis labels and their values
    const xAxisLabels = await page.locator('#SvgjsSvg1245 text').filter({ hasText: /^\d{4}-\d{2}-\d{2}$/ });
    const xAxisValues = [];
    ...
    const barGraphs = await page.locator('#SvgjsSvg1245 path');
