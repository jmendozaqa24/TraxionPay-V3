...
test('Transaction TABLE TEST', async () => {
  ...
  const randomStatusOption = getRandomOption(statusOptions);
  await statusDropdown.selectOption(randomStatusOption);
  const selectedStatusValue = await statusDropdown.inputValue();
  expect(selectedStatusValue).toBe(randomStatusOption);

  // Check the "entries per page" dropdown
  const entriesDropdown = await page.getByLabel('entries per page');
  const entriesOptions = ['10', '25', '50', '100'];
  const randomEntriesOption = getRandomOption(entriesOptions);
  await entriesDropdown.selectOption(randomEntriesOption);
  const selectedEntriesValue = await entriesDropdown.inputValue();
  expect(selectedEntriesValue).toBe(randomEntriesOption);
});
