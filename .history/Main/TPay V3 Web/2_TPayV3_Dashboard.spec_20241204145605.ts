...
test.describe('Dashboard - Analytics', () => {
  test('Overview Analytics Visibility and Retrieval', async () => {
    ...
    const payInsText = await page.getByRole('link', { name: /PAYINS/ }); // Adjust the regex to match the format of the PayIns value
    await expect(payInsText).toBeVisible();
    const payInsTextText = await payInsText.textContent();
    console.log(`Total PayIns: Visible\nPAYINS ${payInsTextText.trim()}`);
