...
test.describe('TPay V3 - User Profile', () => {
  test('User Profile', async () => {
    ...
    const kycStatus = kycStatusElement ? await kycStatusElement.evaluate(node => node.nextElementSibling?.textContent.trim() || 'N/A') : 'N/A';

    const personCodeElement = await page.locator('text=Person Code:').elementHandle();
    const personCode = personCodeElement ? await personCodeElement.evaluate(node => node.nextElementSibling?.textContent.trim() || 'N/A') : 'N/A';

    const mobileNumberElement = await page.locator('text=Mobile Number:').elementHandle();
    const mobileNumber = mobileNumberElement ? await mobileNumberElement.evaluate(node => node.nextElementSibling?.textContent.trim() || 'N/A') : 'N/A';

    const emailElement = await page.locator('text=Email:').elementHandle();
    const email = emailElement ? await emailElement.evaluate(node => node.nextElementSibling?.textContent.trim() || 'N/A') : 'N/A';
    ...
    console.log(`Verification: User is ${isVerified ? 'VERIFIED' : 'NOT VERIFIED'}`);
  });
});