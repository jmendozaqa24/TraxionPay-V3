  ...
  test('Transaction Table Visibility', async () => {
    ...
    test('Transaction Table Entry Page', async () => {
      ...
      for (const option of entriesOptions) {
        ...
        // Validate the number of rows under the "Reference Number" column
        const referenceColumnCells = await page.$$('div#transactions-list_wrapper tbody tr td:has-text("Reference Number")');
        expect(referenceColumnCells.length).toBe(parseInt(option)); // Ensure the number of rows matches the selected entries per page
