...
test.describe('Dashboard - Analytics', () => {
  ...
  test('Transaction Chart Visibility and Retrieval', async ({ page }) => {
    ...
    await page.waitForSelector('div:nth-child(2) > .col-lg-12 > .card > .card-body', { timeout: 10000 });
    const chartSvg = page.locator('div:nth-child(2) > .col-lg-12 > .card > .card-body');
    ...
    for (let i = 0; i < barGraphCount; i++) {
      ...
      if (boundingBox) {
        ...
        console.log(`Bar Graph ${i + 1}: Y-Axis Value = ${yAxisValue}`);
      } else {
