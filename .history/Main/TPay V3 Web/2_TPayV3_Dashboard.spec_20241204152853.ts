...
test.describe('Dashboard - Analytics', () => {
  test('Overview Analytics Visibility and Retrieval', async ({ page }) => {
    ...
    const totalPayIns = await page.locator('#payins-total', { timeout: 20000 }); // Wait up to 20 seconds
    ...
    // Check if the total PayOuts value element is visible using its ID
    console.log('Checking visibility of the total PayOuts value...');
    const totalPayOuts = await page.locator('#payouts-total', { timeout: 20000 }); // Wait up to 20 seconds
    await expect(totalPayOuts).toBeVisible();
    const totalPayOutsText = await totalPayOuts.textContent();
