...
test.describe('Dashboard - Analytics', () => {
  ...
  test('Transaction Chart Visibility and Retrieval', async ({ page }) => {
    ...
    const chartSvg = await page.waitForSelector('div:nth-child(2) > .col-lg-12 > .card > .card-body', { timeout: 10000 });
    ...
    // Wait for bar graphs to be visible and retrieve their bounding boxes
    ...
    for (let i = 0; i < barGraphCount; i++) {
      ...
      // Retrieve and log the bounding box of the bar graph
      const boundingBox = await barGraph.boundingBox();
      if (boundingBox) {
        const barGraphHeight = boundingBox.height;
        const barGraphWidth = boundingBox.width;
        const yAxisValue = yAxisValues.find(value => value >= barGraphHeight);
