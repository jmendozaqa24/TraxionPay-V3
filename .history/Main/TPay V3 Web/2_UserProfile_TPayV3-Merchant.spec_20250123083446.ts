...
test.describe('TPay V3 - User Profile', () => {
  ...
  test('Bank Accounts Tab', async () => {
    ...
    for (let i = 0; i < accountCount; i++) {
        ...
        const bankName = await page.getByText('text=Bank Name:').nextSibling().textContent();
        const accountNumber = await page.getByText('text=Account Number:').nextSibling().textContent();
