...
test('correct login', async ({ page }) => {
  ...
  const [response] = await Promise.all([
    page.waitForResponse(response => response.url().includes('/api/signin') && response.status() === 200),
...
test('incorrect login', async ({ page }) => {
  ...
  // Add on this part the API validation
  const [response] = await Promise.all([
    page.waitForResponse(response => response.url().includes('/api/login') && [400, 404].includes(response.status())),
    page.getByRole('button', { name: 'Sign in' }).click()
  ]);
