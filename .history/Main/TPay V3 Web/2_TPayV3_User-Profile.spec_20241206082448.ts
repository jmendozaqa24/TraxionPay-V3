...
test.describe('TPay V3 - User Profile', () => {
  test('User Profile', async () => {
    ...
    const mobileIsActive = (await getTextContent('IsActive:')).first();
    const mobileIsPrimary = (await getTextContent('IsPrimary:')).first();

    const emailType = await getTextContent('Type: Email Address');
    const emailValue = await getTextContent('Value: ');
    const emailIsActive = (await getTextContent('IsActive:')).nth(1);
    const emailIsPrimary = (await getTextContent('IsPrimary:')).nth(1);
