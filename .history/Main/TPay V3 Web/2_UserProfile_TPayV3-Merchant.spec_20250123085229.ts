...
test.describe('TPay V3 - User Profile', () => {
  ...
  test('Bank Accounts Tab', async () => {
    ...
    for (let i = 0; i < accountContainers.length; i++) {
        ...
        const bankName = await container.$eval('text=Bank Name:', node => node.nextSibling?.textContent?.trim() || '');
        const accountName = await container.$eval('text=Account Name:', node => node.nextSibling?.textContent?.trim() || '');
        const accountNumber = await container.$eval('text=Account Number:', node => node.nextSibling?.textContent?.trim() || '');
        const accountType = await container.$eval('text=Account Type:', node => node.nextSibling?.textContent?.trim() || '');
