...
test.describe('TPay V3 - User Profile', () => {
  test('User Profile', async () => {
    ...
    // Function to retrieve and log text content with delay
    const getTextContent = async (selector: string) => {
      const element = await page.getByText(selector);
      await element.scrollIntoViewIfNeeded();
      await page.waitForTimeout(100); // Add a delay of 500ms
    ...
    await page.getByText('Other Personal Details').scrollIntoViewIfNeeded();
    await page.waitForTimeout(100); // Add a delay of 500ms
    ...
    // User Details
    await page.getByText('User Details').scrollIntoViewIfNeeded();
    await page.waitForTimeout(100); // Add a delay of 500ms
    ...
    // Identification
    await page.getByText('Identifications', { exact: true }).scrollIntoViewIfNeeded();
    await page.waitForTimeout(100); // Add a delay of 500ms
    // Add retrieval and logging for identification details if needed

    // Wallet Details
    await page.getByText('Wallet Details').scrollIntoViewIfNeeded();
    await page.waitForTimeout(100); // Add a delay of 500ms
    ...
    // Address
    await page.getByText('Addresses').scrollIntoViewIfNeeded();
    await page.waitForTimeout(100); // Add a delay of 500ms
