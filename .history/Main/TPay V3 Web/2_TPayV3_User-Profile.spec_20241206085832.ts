...
test.describe('TPay V3 - User Profile', () => {
  test('User Profile', async () => {
    ...
    const userName = await page.locator('h1.fw-bold').textContent();
    const kycStatus = await page.locator('text=KYC Status:').nextSibling().textContent();
    const personCode = await page.locator('text=Person Code:').nextSibling().textContent();
